...
publish: false
description: Anthropic introduces Agent Skills—organized folders of instructions, scripts, and resources that enable Claude agents to dynamically load domain-specific expertise. Skills use progressive disclosure to manage context efficiently, allowing agents to discover and load relevant information only when needed for specific tasks.
excerpt: Discover how Anthropic builds AI agents with practical capabilities through modular skills, enabling them to handle complex real-world tasks more effectively and reliably.
