...
The companion prints `Thread ready (<id>)` as the thread opens, ahead of the work. Give the user `codex://threads/<id>` at that point, so a backgrounded dispatch can be watched in the Codex app while it runs. `review` and `adversarial-review` are ephemeral by design and leave no visible thread, so a missing thread after a review is not a failure. Raw `codex exec` leaves no app-server thread at all.
