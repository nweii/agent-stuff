...
- **Optional `[meta.ts]`**: default-exports a `[LabMeta]` (`[lib/lab-registry.ts]`)
  — title, description, date, status (`tuning`/`settled`/`superseded`), winning
  variant. Absent meta falls back to the folder slug.
...
(status, winner) — the lab now shows production's new defaults, closing the
loop.
```
