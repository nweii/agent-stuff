...
4. **Build the shell**: a layout that renders the sidebar from the registry and sets `noindex`. Sidebar: use the host's existing component library if present, otherwise a plain rail (~60 lines). Host-specific extras (a link to an internal dashboard, "back to main site") go in the sidebar footer, marked as host extensions outside the general pattern.
